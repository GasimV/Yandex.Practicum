...
// Реализация PtrVector с оператором присваивания копии с использованием идиомы copy-and-swap
template <typename T>
class PtrVector {
public:
    PtrVector() = default;
    ...
    // Оператор присваивания копии с использованием идиомы copy-and-swap
    PtrVector& operator=(const PtrVector& rhs) {
        if (this != &rhs) {
            PtrVector temp(rhs); // Создаём копию rhs
            swap(temp); // Поменяем местами this с temp
        }
        return *this;
    }

    // Метод для обмена содержимого двух объектов PtrVector
    void swap(PtrVector& other) noexcept {
        items_.swap(other.items_); // Меняем векторы местами; эта операция не вызывает исключений
    }
...
// Перегружаем функцию swap, чтобы разрешить использование std::swap
template <typename T>
