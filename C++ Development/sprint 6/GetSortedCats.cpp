...
vector<const Cat*> GetSortedCats(const vector<Cat>& cats, const Comparator& comp) {
   ...
   // 1) Помещаем в массив sorted_cat_pointers адреса объектов из массива cats
   for (const Cat& cat : cats) {
    sorted_cat_pointers.push_back(&cat);
   }

   // 2) Отсортируем массив sorted_cat_pointers с помощью переданного компаратора comp
