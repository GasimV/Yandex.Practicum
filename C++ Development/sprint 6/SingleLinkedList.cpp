...
public:
    ...
    // Оператор присваивания с обеспечением строгой гарантии безопасности исключений
    ...
    void PushFront(const Type& value) {
        Node* new_node = new Node(value, head_.next_node); // Создаём новый узел
        head_.next_node = new_node; // Вставляем новый узел после фиктивного узла
    ...
    void Clear() noexcept {
        while (head_.next_node != nullptr) {
            ...
            delete node_to_delete; // Удаляем текущий узел
        }
        size_ = 0; // Сбрасываем размер списка
    }
    ...
    // Удаляет элемент, следующий за указанным итератором
    ...
    // Итератор, указывающий на фиктивный элемент перед первым элементом
