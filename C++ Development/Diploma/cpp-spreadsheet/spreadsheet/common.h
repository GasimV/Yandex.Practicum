...
// Структура позиции ячейки. Индексация с нуля.
struct Position {
    int row = 0;
    int col = 0;
...
// Класс для ошибок вычисления формулы.
class FormulaError {
public:
    enum class Category {
        Ref,        // неверная ссылка
        Value,      // невозможно трактовать как число
        Arithmetic, // арифметическая ошибка (например, деление на ноль)
    };
...
// Интерфейс ячейки
class SheetInterface; // предварительное объявление

class CellInterface {
public:
    // Значение ячейки – либо текст, либо число, либо сообщение об ошибке.
    using Value = std::variant<std::string, double, FormulaError>;

    virtual ~CellInterface() = default;
    virtual Value GetValue() const = 0;
    virtual std::string GetText() const = 0;
    // Возвращает список ячеек, задействованных в формуле (если применимо).
