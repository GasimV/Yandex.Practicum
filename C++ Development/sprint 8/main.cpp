...
struct PlateHasherAll {
    // реализуйте собственный хешер, который будет эффективнее и лучше всех остальных
    size_t operator()(const VehiclePlate& plate) const {
        uint64_t hash = 0;

        // Переменные для хранения кодов букв
        int64_t letter1 = plate.GetLetters()[0] - 'A';
        int64_t letter2 = plate.GetLetters()[1] - 'A';
        int64_t letter3 = plate.GetLetters()[2] - 'A';

        // Комбинируем компоненты номера в единое значение
        // letters_ * большие множители + digits и region
        hash = letter1 * 26 * 26 * 1'000'000ULL + // Первый символ, смещён на два символа и 1,000,000
               letter2 * 26 * 1'000'000ULL +      // Второй символ, смещён на один символ и 1,000,000
               letter3 * 1'000'000ULL +           // Третий символ, множитель для всех цифр
               plate.GetDigits() * 1000 +         // Числовая часть, множитель для региона
               plate.GetRegion();                 // Регион как младшая часть
